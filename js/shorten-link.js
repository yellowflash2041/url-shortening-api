...
const inputLink = document.querySelector("#shorten-it__input-link");
const shortenItBtn = document.querySelector("#shorten-it__btn");

/* shortened links container */
const shortenedLinks = document.querySelector(".shorten-it__links");
...
const noLinkError = document.querySelector("#add-link--error");

let link;
const relAPI = "https://rel.ink/api/links/";

const fetchURL = async (link) => {
  const response = await fetch(relAPI, {
    method: "POST",
    headers: {
  ...
  const json = await response.json(link);
  return {
    url: relAPI + json.hashid,
  };
};

const createShortenedLinkItem = () => {
